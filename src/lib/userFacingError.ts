...
export function toSafeError(error: unknown, context = "save this record"): SafeError {
  ...
  if (DUPLICATE.test(text)) {
    return { title: "Already recorded", description: "A record with these details already exists. Open the existing record instead of creating a duplicate.", canRetry: false, supportReference: ref };
  }
  if (NOT_NULL.test(text)) {
    return { title: "Missing required information", description: "Some required fields are empty. Complete every field marked as required and try again.", canRetry: true, supportReference: ref };
  }
  if (FOREIGN_KEY.test(text)) {
    return { title: "Linked record not found", description: "A record this entry depends on is missing or was archived. Refresh the page and reselect the linked record.", canRetry: true, supportReference: ref };
  }
  if (NETWORK.test(text)) {
    return { title: "Connection problem", description: "We could not reach the server. Check your connection and retry.", canRetry: true, supportReference: ref };
  }
  return {
